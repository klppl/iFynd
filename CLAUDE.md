...
fixed-price ("köp nu") listings against historical sold prices per (model,
storage) bucket. Runs on a VPS via docker-compose. No CGO
(modernc.org/sqlite). The category/family pipeline is deliberately generic
so another device family (a re-added iPad/MacBook, Watch, …) can be plugged
back in — see `internal/classify`.
...
- **internal/classify** — (model, storage) bucketing, per family. Today the
  only family is `IPhone`, but `Family`/`ParseFamily`/`junkByFamily` and the
  per-family dispatch in `Item` stay an extensible enum: a new category is a
  constant, a `junkByFamily` entry, and a branch in `Item`. (iPad and MacBook
  families were removed — the laptop category was multi-brand and never
  classified reliably; the git history has the old ipad.go/macbook.go if
  they're wanted back.) iPhones prefer Tradera's structured attributes
  (`mobile_model`, `mobile_disk_memory`, `condition`) with title regexes as
  fallback. Rejects accessories/bundles/broken/ambiguous listings with a
  reason; those land in the `skipped_listings` table for auditing. Raw titles
